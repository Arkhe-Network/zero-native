...
def train_epoch(model, dataloader, optimizer, criterion, device, epoch):
    # model.train() is shadowed by the model's custom train method, but nn.Module.train(self) is what we need
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        # We need a dummy parameter that requires grad in order to test backward in this stub
        predictions = {
        ...
        # Emulate a forward pass output that depends on model parameters
        predictions["logits"] = predictions["logits"] + sum(p.sum() for p in model.parameters()) * 0.0
        predictions["state_pred"] = predictions["state_pred"] + sum(p.sum() for p in model.parameters()) * 0.0
        targets = {
            "tokens": tokens,
