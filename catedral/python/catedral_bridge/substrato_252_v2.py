...
"""
SDX-ARKHE v2.0 — Software Distribution Ontology

NOVAS CLASSES (v2.0):
  sdx:OCIImage         → Imagem de container OCI
  sdx:HelmChart        → Chart Helm para Kubernetes
  sdx:SLSAProvenance   → Provenance com assinatura SLSA
  sdx:CICDBuild        → Instância de build do pipeline CI/CD (881)

NOVAS PROPRIEDADES DE OBJETO (v2.0):
  sdx:builtBy          → Liga artefato ao build que o produziu
  sdx:hasSLSAProvenance → Liga ao attest SLSA
  sdx:chainDependsOn   → Dependência transitiva com verificação de selo

NOVAS PROPRIEDADES DE DADOS (v2.0):
  sdx:digest, sdx:ociMediaType, sdx:chartVersion, sdx:chartAppVersion,
  sdx:slsaLevel, sdx:buildRunID, sdx:buildCommit, sdx:buildSigner

CHAIN OF TRUST:
  Verifica recursivamente que cada dependência carrega o selo arkhe:hasSeal
  correspondente ao registro canônico da Catedral.
"""
...
class SDXArkheV2:
    ...
    def create_oci_image(self, name: str, version: str, digest: str, media_type: str,
        ...
        if build_info:
            artifact["sdx:builtBy"] = {"@type": "sdx:CICDBuild", **build_info}
        if slsa:
            artifact["sdx:hasSLSAProvenance"] = {"@type": "sdx:SLSAProvenance", **slsa}
        if dependencies:
    ...
    def register_seal(self, artifact_name: str, seal_hash: str):
        """Registra o selo canônico de um artefato na Catedral."""
        self.known_seals[artifact_name] = seal_hash

    def verify_chain_of_trust(self, artifact: dict, prefix_len: int = 8) -> dict:
        """Verifica a cadeia de dependências contra o registry da Catedral."""
        results = []
        all_valid = True
...
if __name__ == "__main__":
    ...
    # Registrar selos canônicos da Catedral
    ...
    print("OCI Image:", sdx.to_jsonld(oci)[:200] + "...")
    print("\nHelm Chart Chain Verification:", sdx.verify_chain_of_trust(helm))