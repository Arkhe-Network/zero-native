...
║  Extensão OCI/Helm + Build Pipeline + Chain of Trust             ║
...
class OCIImage:
    ...
    def to_sdx(self) -> dict:
        return {
            "@type": "sdx:OCIImage",
            "sdx:artifactName": f"{self.name}:{self.version}",
            "sdx:digest": self.digest,
            "sdx:ociMediaType": self.media_type,
            "sdx:publishedAt": {"sdx:repositoryURL": f"{self.registry}/{self.name}"},
...
class SLSAProvenanceGenerator:
    ...
    def generate(artifact: dict, pipeline_run_id: str,
        ...
        return {
            ...
            "sdx:buildEnvironment": f"ARKHE CI/CD — run {pipeline_run_id}",
...
if __name__ == "__main__":
    ...
    chart = HelmChart(
        ...
        dependencies=[
            {"name": "arkhe-gateway", "constraint": ">=870-G.4.0"}
        ],
        seal="f1e2d3c4b5a6..."
    )

    # Gera proveniência SLSA
    prov = SLSAProvenanceGenerator.generate(img.to_sdx(), "run-4242")
    img.to_sdx()["sdx:hasProvenance"] = prov
    ...
    cot = ChainOfTrust()
    valid = cot.verify(img.to_sdx(), registry)
    print(f"Chain of Trust válida: {valid}")